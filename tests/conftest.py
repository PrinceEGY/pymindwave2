...
CONNECTED_DATA = {
    "rawEeg": b'{"rawEeg":%d}\r',
    "eSense": b'{"eSense":{"attention":%d,"meditation":%d},"eegPower":{"delta":%d,"theta":%d,"lowAlpha":%d,"highAlpha":%d,"lowBeta":%d,"highBeta":%d,"lowGamma":%d,"highGamma":%d},"poorSignalLevel":%d}\r',
...
def mock_tg_connector(mocker):
    class MockThinkGearConnector:
        def __init__(self, scan_count=10, scan_sleep_duration=0.5):
            ...
            mocker.patch(
                "mindwave.connector.ThinkGearConnector.disconnect", side_effect=self._disconnect
            )

        def _prepare_data_queue(self):
            data_types = (
                ["rawEeg"] * 512 + ["eSense"] * 1 + ["blinkStrength"] * random.randint(0, 2)
            )
        ...
        async def _connect(self):
            self.tg_connector.st_writer = mocker.Mock()
            self.tg_connector.st_reader = mocker.AsyncMock()
    ...
    def create_mock(scan_count=0, scan_sleep_duration=0.5):
        return MockThinkGearConnector(scan_count, scan_sleep_duration).get_connector()
